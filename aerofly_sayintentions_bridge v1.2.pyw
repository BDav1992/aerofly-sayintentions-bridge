...
class AeroflyToSayIntentionsUI:
    ...
    def _init_state(self):
        ...
        self.pending_dest_weather = None  # destination weather - only staged in 'Don't load' mode
    ...
    def _build_tab_simbrief(self):
        ...
        flight_plan_mode_combo = ttk.Combobox(
            ...
            values=["Full load", "Pre-load", "Empty", "Don't load"], width=12, state="readonly"
        )
        ...
        ttk.Label(
            ...
            text="Note: Full load stages every navlog waypoint. Pre-load stages only origin/destination\n"
                 "+ runways. Empty clears the route entirely. Real time is always staged regardless of\n"
                 "mode. Don't load leaves aircraft/route completely untouched (e.g. resuming a flight\n"
                 "already in progress) and stages the destination's weather instead of the departure's -\n"
                 "every other mode stages departure weather.",
    ...
    def _fetch_and_parse_metar(self, icao):
        ...
        raw = m.get("rawOb", "---")
        # wdir is usually a number, but METAR reports variable wind as the literal
        # string "VRB" (no single heading) - fall back to 0 rather than crashing.
        try:
        ...
        # Turbulence estimate: METAR has no direct turbulence value, but gust factor
        # (how much stronger gusts are than sustained wind) is a reasonable proxy for
        # low-level mechanical turbulence - a 20kt+ gust spread is treated as "as
        # turbulent as this model gets" (1.0), scaling down from there.
        try:
            gust_kt = float(m.get("wgst") or 0)
        except (TypeError, ValueError):
            gust_kt = 0.0
        gust_spread_kt = max(gust_kt - wind_kt, 0.0)
        turbulence_pct = min(gust_spread_kt / 20.0, 1.0)

        # Thermal activity estimate: no direct METAR field either. Convective cloud
        # types (CB/TCU) called out in the raw text are a strong signal; short of
        # that, a warm temperature with some cumulus-type cover is a weaker one.
        # Best-effort only - not a real convection forecast.
        if "TCU" in raw or "CB" in raw:
            thermal_activity_pct = 0.9
        elif temp_c is not None and temp_c >= 20 and cloud_density > 0:
            thermal_activity_pct = 0.4
        else:
            thermal_activity_pct = 0.1

        return {
            'raw': raw, 'wind_dir': wind_dir, 'wind_kt': wind_kt, 'gust_kt': gust_kt,
            ...
            'turbulence_pct': turbulence_pct, 'thermal_activity_pct': thermal_activity_pct,
        }
    ...
    def fetch_weather_metar(self):
        ...
        try:
            ...
            self.pending_weather = {
                ...
                'turbulence_pct': w['turbulence_pct'], 'thermal_activity_pct': w['thermal_activity_pct'],
            }

            self.update_ui("wx_raw", w['raw'])
            self.update_ui("wx_wind", f"{w['wind_dir']:.0f}° @ {w['wind_kt']:.0f} kt" + (f" (gust {w['gust_kt']:.0f})" if w['gust_kt'] else ""))
    ...
    def fetch_destination_metar_async(self, icao):
        """Fetches the destination airport's METAR for informational display, and stores it
        for potential staging - normally it stays reference-only (departure weather is what
        gets staged), except in Flight plan mode 'Don't load', which stages this instead.
        Runs on a background thread since this fires automatically right after a SimBrief
        fetch, not from a direct button click."""
        def worker():
            try:
                w = self._fetch_and_parse_metar(icao)
                self.pending_dest_weather = w
    ...
    def _display_destination_metar(self, w):
        ...
        self.update_ui("dest_wx_wind", f"{w['wind_dir']:.0f}° @ {w['wind_kt']:.0f} kt" + (f" (gust {w['gust_kt']:.0f})" if w['gust_kt'] else ""))
    ...
    def launch_aerofly(self, vr):
        ...
        mcf_path = self.find_mcf_path()
        if not mcf_path:
            messagebox.showerror("Error", "Could not find main.mcf - use 'Browse main.mcf...' in the bar at the bottom of the window to select it manually.")
            return
        ...
        try:
            ...
            # "Don't load" (resuming a flight already in progress) stages the destination's
            # weather instead of the departure's, since departure conditions are no longer
            # relevant - every other mode stages departure weather as usual.
            weather_to_apply = self.pending_dest_weather if flight_plan_mode == "Don't load" else self.pending_weather
            ...
            if weather_to_apply:
                ...
                self.replace_mcf_value(lines, "turbulence", f"{w['turbulence_pct']:.4f}")
                self.replace_mcf_value(lines, "thermal_activity", f"{w['thermal_activity_pct']:.4f}")
                applied_parts.append("weather (destination)" if flight_plan_mode == "Don't load" else "weather")

            if flight_plan_mode == "Don't load":
                pass  # aircraft/route left completely untouched, on purpose
            else:
